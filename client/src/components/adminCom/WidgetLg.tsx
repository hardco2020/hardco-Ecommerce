...
const Button = styled.button<ButtonProp>`
    ...
    border: none;
    border-radius: 10px;
    background-color: ${p=>(p.status==="Approved" && "#e5faf2") || (p.status==="Declined" && "#fff0f1") || (p.status==="Pending" && "#ebf1fe")};
    color:${p=>(p.status==="Approved" && "#3bb077") || (p.status==="Declined" && "#d95087") || (p.status==="Pending" && "#2a7ade")};
`
const WidgetLg = () => {
    return (
        <Container>
